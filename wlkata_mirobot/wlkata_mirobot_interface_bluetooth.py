...
from .wlkata_mirobot_exceptions import MirobotError, MirobotAlarm, MirobotReset, InvalidBluetoothAddressError
...
class BluetoothLowEnergyInterface:
	...
	def __init__(self, mirobot, address=None, debug=False, logger=None, autofindaddress=True):
		...
		Parameters
		----------
		mirobot : `mirobot.wlkata_mirobot_gcode_protocol.WlkataMirobotGcodeProtocol`
		...
		logger : Logger
			(Default value = None) Logger instance to use for this class. Usually `mirobot.wlkata_mirobot_gcode_protocol.WlkataMirobotGcodeProtocol.logger`.
		...
		Returns
		-------
		class : `mirobot.wlkata_mirobot_interface_bluetooth.BluetoothLowEnergyInterface`
		"""
	...
	async def _ainit(self, address=None, autofindaddress=True):
		...
		if not address:
			if autofindaddress:
				...
				""" The default address to use when making connections. To override this on a individual basis, provide portname to each invocation of `WlkataMirobotGcodeProtocol.connect`. """
	...
	def debug(self, value):
		"""Set the new value for the `debug` property of `mirobot.wlkata_mirobot_interface_bluetooth.BluetoothLowEnergyInterface`. Use as in `BluetoothLowEnergyInterface.setDebug(value)`.
